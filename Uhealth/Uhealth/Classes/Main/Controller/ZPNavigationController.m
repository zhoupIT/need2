...
@implementation UIViewController (ZPNavigationControllerAdd)
...
typedef NS_ENUM(int, ZPNavMovingStateEnumes) {
    ZPNavMovingStateStanby = 0,
    ZPNavMovingStateDragBegan,
    ZPNavMovingStateDragChanged,
    ZPNavMovingStateDragEnd,
    ZPNavMovingStateDecelerating,
};
 ...
 *  正在移动
 */
@property (nonatomic,assign) ZPNavMovingStateEnumes movingState;
...
{
    // 设置水平位移在 [0, ZPNavViewW] 之间
    x = MAX(MIN(x, ZPNavViewW), 0);
    // 设置frame的x
    self.view.frame = (CGRect){ {x, self.view.frame.origin.y}, self.view.frame.size};
    // 设置黑色蒙版的不透明度
    self.lastScreenBlackMask.alpha = 0.6 * (1 - (x / ZPNavViewW));
    // 设置上一个截屏的缩放比例
    CGFloat scale = x / ZPNavViewW * 0.05 + 0.95;
...
- (void)paningGestureReceive:(UIPanGestureRecognizer *)recoginzer
{
    ...
    if (UIGestureRecognizerStateBegan == recoginzer.state) {
        if (self.movingState == ZPNavMovingStateStanby) {
            self.movingState = ZPNavMovingStateDragBegan;
        ...
        if (self.movingState == ZPNavMovingStateDragBegan || self.movingState == ZPNavMovingStateDragChanged) {
            self.movingState = ZPNavMovingStateDragEnd;
            [self panGestureRecognizerDidFinish:recoginzer];
        }
    } else if (recoginzer.state == UIGestureRecognizerStateChanged) {
        if (self.movingState == ZPNavMovingStateDragBegan || self.movingState == ZPNavMovingStateDragChanged) {
            self.movingState = ZPNavMovingStateDragChanged;
            [self moveViewWithX:[recoginzer translationInView:ZPKeyWindow].x];
        }
    }
}
...
#define decelerationTime (0.4)
    // 获取手指离开时候的速率
    CGFloat velocityX = [panGestureRecognizer velocityInView:ZPKeyWindow].x;
    // 手指拖拽的距离
    CGFloat translationX = [panGestureRecognizer translationInView:ZPKeyWindow].x;
    // 按照一定decelerationTime的衰减时间，计算出来的目标位置
    CGFloat targetX = MIN(MAX(translationX + (velocityX * decelerationTime / 2), 0), ZPNavViewW);
    // 是否pop
    BOOL pop = ( targetX > ZPMinX );
    // 设置动画初始化速率为当前瘦子离开的速率
    CGFloat initialSpringVelocity = fabs(velocityX) / (pop ? ZPNavViewW - translationX : translationX);
    
    self.movingState = ZPNavMovingStateDecelerating;
    ...
    [UIView animateWithDuration:ZPAnimationDuration
                          delay:0
         usingSpringWithDamping:1
          initialSpringVelocity:initialSpringVelocity
                        options:UIViewAnimationOptionCurveEaseOut
                     animations:^{
                         [self moveViewWithX:pop ? ZPNavViewW : 0];
                         ...
                         self.movingState = ZPNavMovingStateStanby;
