...
- (void)setStyle:(PopoverViewStyle)style {
    ...
    if (_style == PopoverViewStyleDefault) {
        if (self.selStr.length && _button != nil && [self.selStr isEqualToString:_button.titleLabel.text]) {
            [_button setTitleColor:RGB(255,150,0) forState:UIControlStateNormal];
        } else {
             [_button setTitleColor:ZPMyOrderDetailFontColor forState:UIControlStateNormal];
        }
    } else {
...
- (void)initialize {
    ...
    // Constraint
    [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-9-[bottomLine]-9-|" options:kNilOptions metrics:nil views:NSDictionaryOfVariableBindings(bottomLine)]];
...
+ (UIColor *)bottomLineColorForStyle:(PopoverViewStyle)style {
    return style == PopoverViewStyleDefault ? RGB(238,238,238) : [UIColor colorWithRed:0.4 green:0.4 blue:0.4 alpha:1.00];
}
